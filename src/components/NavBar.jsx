                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <div key={item.id} className={navbarStyles.desktopNavItem}>
                                <NavLink to={item.path} end className={({isActive}) => `${navbarStyles.desktopNavLink.base} ${isActive ? navbarStyles.desktopNavLink.active : navbarStyles.desktopNavLink.inactive}`}>
            ...
            <div className={navbarStyles.rightSection}>
                <div className= {navbarStyles.tabletNav}>
                    {navItems.map((item) => {
                      const Icon = item.icon;
                      return (
                        <NavLink key={item.id} to={item.path} end className={({isActive}) => `${navbarStyles.tabletNavLink.base} ${isActive ? navbarStyles.tabletNavLink.active :
                         navbarStyles.tabletNavLink.inactive}`}>
                          <Icon className= {navbarStyles.tabletNavIcon}></Icon>
                  ...
                  <div className={navbarStyles.desktopAuth}>
                    {isLoggedIn ? (
                      <button title = {userEmail || "Logout"} onClick={handleLogOut} className={navbarStyles.logoutButton}>
                        <LogOut className={navbarStyles.logoIcon} />
                        <span>Logout</span>
                      </button>
                    ) : (
                ...
                <div className= {navbarStyles.mobileMenuItems}>
                  {navItems.map((item) => {
                      ...
                      return (
                        ...
                        className={({isActive}) => `${navbarStyles.mobileNavLink.base} ${isActive ? navbarStyles.mobileNavLink.active : navbarStyles.mobileNavLink.inactive}`}>
                          <Icon className= {navbarStyles.mobileNavIcon}></Icon>
